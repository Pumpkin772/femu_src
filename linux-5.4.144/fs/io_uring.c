				 ...
				 long res)
{
	...
	if (cqe) {
		...
		//printk("013:cqe->usr_data:%llu",ki_user_data);
...
{
	struct io_kiocb *req = container_of(kiocb, struct io_kiocb, rw);//通过偏移量获得结构体成员
	//add-zhou
	ktime_get_ts64(&req->end_time);
	//add
	struct io_uring_cqe *cqe;
	...
	/*io_cqring_add_event函数将事件添加到完成队列（CQ）中。这个函数通常会将完成的 I/O 事件传递给用户空间*/
	//io_cqring_add_event(req->ctx, req->user_data, res);
	//add-zhou
	unsigned long starttime = req->start_time.tv_sec*1000000000+req->start_time.tv_nsec;
	unsigned long endtime = req->end_time.tv_sec*1000000000+req->end_time.tv_nsec;
	unsigned long delaytime = endtime - starttime;
	res = (__s32) delaytime; 
	io_cqring_add_event(req->ctx, req->rw.ki_usrflag, res);
	printk("013:io_complete_rw:cqe->usr_data:%llu\n",req->rw.ki_usrflag);
	//cqe = io_get_cqring(req->ctx);
	//if(cqe)
	//{
	//	printk("013:cqe->usr_data:%llu",cqe->user_data);
	//}
	//add
	io_put_req(req);
}
...
static void io_complete_rw_iopoll(struct kiocb *kiocb, long res, long res2)
{
	...
	io_cqring_add_event(req->ctx, req->rw.ki_usrflag, res);
	printk("013:io_complete_rw_iopoll:cqe->usr_data:%llu\n",req->rw.ki_usrflag);
}
		      ...
		      bool force_nonblock)
{
	...
	//zhr-传递usr_flag到kiocb中
	kiocb->ki_usrflag = READ_ONCE(sqe->usr_flag);
	printk("002:kiocb->ki_usrflag:%llu",kiocb->ki_usrflag);
		   ...
		   bool force_nonblock)
{
	...
	ret = io_prep_rw(req, s, force_nonblock);//zhr-将sqe中的信息传到req->rw(kiocb)中
	if (ret)
		return ret;
	file = kiocb->ki_filp;
	...
	if (!ret) {
		...
		if (file->f_op->read_iter)
			ret2 = call_read_iter(file, kiocb, &iter);//zhr-在下层，读数据
			  ...
			  struct io_submit_state *state, struct io_kiocb **link)
{
	...
	ktime_get_ts64(&req->start_time);//add-zhou
...
static int io_sq_thread(void *data)
{
	...
	struct mm_struct *cur_mm = NULL;//zhr-当前进程的内存管理结构
	const struct cred *old_cred;
	mm_segment_t old_fs;
	DEFINE_WAIT(wait);
	unsigned inflight;//inflight表示已经提交但尚未完成的 I/O 请求数量
	unsigned long timeout;

	complete(&ctx->sqo_thread_started);//通过调用 complete() 发出一个完成信号，表示守护线程已经启动
	...
	timeout = inflight = 0;
	/*检查当前内核线程是否应该进入停放状态。停放状态是一种特殊状态，表示线程应该停止执行但不退出。
	它通常用于资源管理和调度控制，以便线程可以在稍后被重新唤醒和恢复执行*/
	while (!kthread_should_park()) {
		bool mm_fault = false;//内存管理的错误
		unsigned int to_submit;

		if (inflight) { 
			unsigned nr_events = 0;//在一次轮询或检查过程中完成的 I/O 请求数量，用于更新 inflight
			...
			if (ctx->flags & IORING_SETUP_IOPOLL) {
				 ...
				 */
				mutex_lock(&ctx->uring_lock);
				if (!list_empty(&ctx->poll_list))//检查是否有已经提交但尚未完成的请求
		...
		to_submit = io_sqring_entries(ctx);//获取当前提交队列中的请求数
		if (!to_submit) {
			/*
		...
		/*将待提交的请求从 Submission Queue (SQ) 中取出，并通过
		系统调用将它们提交到内核进行处理*/
	...
	kthread_parkme();//线程退出主循环并调用 kthread_parkme() 进入停放状态
...
static int io_ring_submit(struct io_ring_ctx *ctx, unsigned int to_submit)
{
	...
	for (i = 0; i < to_submit; i++) {
		...
		if (!io_get_sqring(ctx, &s))//zhr-从提交队列中获取下一个请求并存储在 s 中
			break;
...
{	//zhr-此函数的作用是启动一个独立的内核线程来处理提交队列的请求
	int ret;

	mmgrab(current->mm);// 增加当前进程的内存描述符引用计数
	ctx->sqo_mm = current->mm;// 保存当前进程的内存描述符
	...
	if (ctx->flags & IORING_SETUP_SQPOLL) {
		...
		if (p->flags & IORING_SETUP_SQ_AFF) {// 如果设置了SQ线程绑定到特定CPU
			...
			if (!cpu_online(cpu))
				goto err;
			/*io_sq_thread是这个独立线程的主函数，负责实际处理提交队列的逻辑*/
...
{	//zhr-此函数负责实际创建和初始化 io_uring 上下文，包括分配内存、设置参数和启动相关线程
...
{	//zhr- 此函数是用户空间与内核空间交互的入口
	struct io_uring_params p;
	long ret;
	int i;
	//zhr-从用户空间复制 io_uring 参数到内核空间
	if (copy_from_user(&p, params, sizeof(p)))
		return -EFAULT;
	//zhr- 检查保留字段是否被使用，如果被使用则返回错误
