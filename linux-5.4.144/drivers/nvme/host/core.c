...
{	//zhr-request为块设备请求
	...
	//add-zhou-传递usr_flag到nvme命令的保留字段

	if(req && req->bio)
	{
		cmnd->rw.rsvd2 = READ_ONCE(req->bio->bi_usrflag);
		//printk("004:req->bio->bi_usrflag:%llu\n",req->bio->bi_usrflag);
		printk("004:cmnd->rw.rsvd2:%llu\n",cmnd->rw.rsvd2);
		...
		struct nvme_command *cmd)
{
	...
	case REQ_OP_READ:
	case REQ_OP_WRITE:
		ret = nvme_setup_rw(ns, req, cmd);//zhr-在读写请求里传递bi_usrflag
		break;
	default:
		WARN_ON_ONCE(1);
		return BLK_STS_IOERR;
